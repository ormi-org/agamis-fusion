package io.agamis.fusion.core.db.datastore.document

import io.agamis.fusion.core.db.datastore.document.aggregation.typed.Pipeline
import io.agamis.fusion.core.db.datastore.document.aggregation.typed.file.GetFileChildrenFromId
import io.agamis.fusion.core.db.datastore.document.aggregation.typed.file.GetFileFromId
import io.agamis.fusion.core.db.datastore.document.aggregation.typed.file.GetFileFromPath
import io.agamis.fusion.core.db.datastore.document.aggregation.typed.file.GetFilesFromId
import io.agamis.fusion.core.db.datastore.document.exception.typed.file.DuplicatedFileException
import io.agamis.fusion.core.db.datastore.document.exception.typed.file.FileNotFoundException
import io.agamis.fusion.core.db.datastore.typed.DocumentStore
...
class FileStore(collectionSuffix: String)(implicit
    wrapper: ReactiveMongoWrapper
) extends DocumentStore[File] {
    ...
    /** A method for inserting a new file in database
      ...
      *
      * @note
      *   This methods is Async and returns a [[scala.concurrent.Future Future]]
      *   which resolves a [[WriteResult WriteResult]]
      *
      * @param file
      *   the file to insert
      * @param ec
      ...
      * @return
      *   a future [[WriteResult WriteResult]] which attests the acutal creation
      *   of the file
      */
    override def insert(
        file: File
    ...
    /** A method for inserting several files in database
      ...
      * @note
      *   This methods is Async and returns a [[scala.concurrent.Future Future]]
      *   which resolves a [[scala.Int Int]]
      *
      * @param files
      *   the files to insert
      * @param ec
      ...
      *   parallelize computing
      * @return
      *   a future integer which attests the acutal creation of the file
      */
    ...
    /** A method for updating an existing file in database
      ...
      *   parallelize computing
      * @return
      *   a future [[Option Option]] of [[File File]] which reflect new file
    ...
    override def update(
        ...
        wrapper
            ...
            .transformWith({
                case Success(col) =>
                    col.findAndUpdate[BSONDocument, File](
            ...
            .transformWith({
                case Success(col) =>
                    ...
                    val bulkUpdateResult =
                        Future.sequence(updates).flatMap { ops =>
                            updateBuilder.many(ops)
                        }
                    bulkUpdateResult.transformWith({
                        case Success(result) =>
                            if (result.ok) {
                                Future.successful(
                                  result.upserted.asInstanceOf[Seq[File]].toList
                                )
                            } else {
                                throw new Exception(result.errmsg.get)
                            }
    ...
    /** A method for deleting an existing file in database
      ...
      *   parallelize computing
      * @return
      *   a future [[Option Option]] of [[File File]] which reflects the file
      *   state that has been removed from database
      */
    override def delete(
        file: File
    )(implicit ec: ExecutionContext): Future[Option[File]] = {
        wrapper
            ...
            .transformWith({
                case Success(col) =>
                    col.findAndRemove[BSONDocument](
                      BSONDocument("_id" -> file.id)
                    ).map(_.result[File])
                case Failure(cause) => throw cause
            })
    ...
    /** A method for deleting several existing files in database
      ...
      *   parallelize computing
      * @return
      *   a future [[Int Int]] which reflects the count of deleted files
      */
    ...
    /** A method for executing MongoDB aggregation
      *
      * @param pipeline
      *   the parametarized pipeline to execute
      * @param ec
      *   the '''implicit''' [[ExecutionContext ExecutionContext]] used to
      *   parallelize computing
      * @return
      *   a future [[List List]] of [[File File]] which reflects the state of
      *   fetched files in database
      */
            ...
            .transformWith({
                case Success(col) =>
                    col.aggregatorContext[File](
                      pipeline = pipeline.get.asInstanceOf[List[
                        col.AggregationFramework.PipelineOperator
                      ]]
                    ).prepared
                        .cursor
                        .collect[List]()
    ...
    /** A method for retrieving a specific file by its id
      *
      * @note
      *   based on aggregation, it calls aggregate(Pipeline)
      *
      * @param id
      ...
      *   parallelize computing
      * @return
      *   a future [[File File]] which reflects the state of fetched file in
      *   database
      */
    def findByID(id: String)(implicit ec: ExecutionContext): Future[File] = {
        wrapper
            ...
            .transformWith({
                  case Success(col) =>
                      aggregate(pipeline =
                          GetFileFromId.pipeline(col).setId(id)
                      ).transformWith({
                          case Success(files) =>
                              files.length match {
                                  case 0 =>
                                      Future.failed(
                                        FileNotFoundException(
                                          "Couldn't find file with specified id"
                                        )
                                      )
                                  ...
                                  case _ =>
                                      Future.failed(
                                        DuplicatedFileException(
                                          "File id duplication issue"
                                        )
                                      ) // TODO : changer pour une custom
                              }
                          case Failure(cause) =>
                              Future.failed(
                                new Exception("bla bla bla", cause)
                              ) // TODO : changer pour une custom
                      })
                  case Failure(cause) =>
                      Future.failed(
    ...
    def findByPath(
        ...
        wrapper
            ...
            .transformWith({
                case Success(col) =>
                    aggregate(pipeline =
                        GetFileFromPath.pipeline(col).setPath(path)
                    ).transformWith({
                        case Success(files) =>
                            files.length match {
                                case 0 =>
                                    Future.failed(
                                      FileNotFoundException(
                                        "Couldn't find file with specified path"
                                      )
                                    )
                                case 1 => Future.successful(files.head)
                                case _ =>
                        ...
                        case Failure(cause) =>
                            Future.failed(
                              new Exception(
                                "findByPath pipeline execution failure",
                                cause
                              )
                            ) // TODO : changer pour une custom
                    })
                case Failure(cause) =>
                    Future.failed(
                      new Exception("Failed to get collection", cause)
                    ) // TODO : changer pour une custom
            })
    }
    ...
    /** A method for retrieving specific files by their ids
      ...
      *
      * @param ids
      *   the list of ids of files to fetch
      * @param ec
      *   the '''implicit''' [[ExecutionContext ExecutionContext]] used to
      *   parallelize computing
      * @return
    ...
    /** A method for retrieving specific children files by their parent ref
      ...
      *
      * @param dir
      *   the directory to search file in
      * @param ec
      *   the '''implicit''' [[ExecutionContext ExecutionContext]] used to
      *   parallelize computing
      * @return
                ...
                case Success(col) =>
                    dir.path match {
                        ...
                        case None =>
                            Future.failed(
                              new Exception(
                                "Couldn't parse value 'path' from File object"
                              )
                            ) // TODO : changer pour une custom
                    }
                case Failure(cause) =>
