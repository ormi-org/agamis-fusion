...
import java.io.File
import io.agamis.fusion.apps.common.enums.NodeReturnType
import io.agamis.fusion.apps.common.enums.VOID
import java.util.concurrent.atomic.AtomicBoolean
import io.agamis.fusion.apps.engine.exceptions.InstantiateException
import com.caoccao.javet.values.reference.V8ValueObject

/** An object to take care of NodeJs Applications lifecycle inside Javet V8
  * engine
  *
  * @param _logger
  ...
  */
class JavetContainer[T] protected[engine] (implicit
    _logger: Logger,
    ...
    private var _runtime: NodeRuntime =
        new JavetEnginePool(config).getEngine().getV8Runtime()
...
object JavetContainer {
    ...
    /** Instantiate a new container running nodejs script meant to return
      * provided type
      *
      * @param script
      *   the script file to run
      * @param returnType
      *   return type (default is VOID)
      * @param _logger
      *   a slf4j logger instance
      * @param config
      *   a JavetEngine configuration
      * @return
      *   container of a javet instance running provided script
      * @throws InstantiateException
      */
    def ofScriptFile(script: File, returnType: NodeReturnType = VOID)(implicit
        _logger: Logger,
        config: JavetEngineConfig
    ): JavetContainer[_] = {
        val infoMsg =
            ">> JavetContainer#ofScriptFile(File, NodeReturnType) > Running NodeJS script from file:`%s`"
        val errMsg =
            "<< JavetContainer#ofScriptFile(File, NodeReturnType) > Failed running NodeJS script from file:`%s`"
    ...
    /** Instantiate a new container running nodejs process (eg: web server)
      ...
      * @throws InstantiateException
      */
    def ofProcessFile(process: File, env: Map[String, String])(implicit
        _logger: Logger,
        config: JavetEngineConfig
    ): JavetProcessContainer = {
        val infoMsg =
            ">> JavetContainer#ofProcessFile(File) > Running NodeJS process from file:`%s`"
        val errMsg =
            "<< JavetContainer#ofProcessFile(File) > Failed running NodeJS process from file:`%s`"
        ...
        container.run(() => {
            ...
            try {
                ...
                case e: JavetException => {
                    ...
                    throw InstantiateException(
                      String.format("Failed instantiate `%s`", filePath),
                      e
                    )
                }
            }
