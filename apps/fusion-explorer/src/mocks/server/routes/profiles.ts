...
const profilesRoutes = [
  rest.get(
    ...
    (req, res, ctx) => {
      ...
      switch (order[0]) {
        ...
        case 'firstName':
          sortingFn = (a, b) => {
            if (a[order[0]] < b[order[0]])
              return -1 * Number.parseInt(order[1]);
        ...
        default:
          sortingFn = (a, b) =>
            a[order[0]] - b[order[0]] * Number.parseInt(order[1]);
      }
  ...
  rest.get(
    ...
    (req, res, ctx) => {
      const { id } = req.params;
      // return dynamic result
      const fetched = collection.find((p) => p.id === id);

      if (!fetched) {
        return res(ctx.delay(200), ctx.status(404));
      }
      return res(ctx.delay(200), ctx.status(200), ctx.json(fetched));
    }
  ),
  rest.put(
    `/api/organizations/${MOCKED_ORGANIZATION_ID}/profiles/:id`,
    async (req, res, ctx) => {
      ...
      return await lastValueFrom(
        from(
          ...
          switchMap((body) => {
            const { alias, lastName, firstName, isActive } = body;
            const index = collection.findIndex((p) => p.id === id);
            if (index === -1) {
              return of(res(ctx.status(404)));
            }
            const updated = {
              ...collection[index],
              alias,
            ...
            return of(res(ctx.delay(200), ctx.status(200), ctx.json(updated)));
          })
        )
      );
    }
