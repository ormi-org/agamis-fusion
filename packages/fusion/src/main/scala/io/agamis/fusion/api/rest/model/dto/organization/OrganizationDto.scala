...
import io.agamis.fusion.api.rest.model.dto.application.OrganizationApplication
import io.agamis.fusion.api.rest.model.dto.common.JsonFormatters._
import io.agamis.fusion.api.rest.model.dto.filesystem.FileSystemDto
import io.agamis.fusion.api.rest.model.dto.group.GroupDto
import io.agamis.fusion.api.rest.model.dto.organizationtype.OrganizationTypeDto
...
object OrganizationDto {
    def from(o: Organization): OrganizationDto = {
        OrganizationDto(
          Some(o.id),
          o.label,
          o.`type`.collect { case ot: OrganizationType =>
              OrganizationTypeDto.from(ot)
          },
          o.queryable,
          Some(
            o.relatedProfiles
          ...
          Some(
            o.relatedGroups.filter(_._1 == true).map(r => GroupDto.from(r._2))
          ),
          o.defaultFileSystem.collect { case fs: FileSystem =>
              FileSystemDto.from(fs)
          },
...
trait OrganizationJsonSupport
    ...
    import io.agamis.fusion.api.rest.model.dto.organizationtype.OrganizationTypeJsonProtocol._
    import io.agamis.fusion.api.rest.model.dto.application.ApplicationJsonProtocol._
    import io.agamis.fusion.api.rest.model.dto.group.GroupJsonProtocol._
    import io.agamis.fusion.api.rest.model.dto.filesystem.FileSystemJsonProtocol._

    implicit object OrganizationApplicationRelationFormat
        extends JsonFormat[(OrganizationApplication.Status, ApplicationDto)] {
        def write(
            relation: (
                    ...
                    (
                      elements(0) match {
                          case JsString(status) =>
                              status match {
                                  case "ENABLED" =>
                                      OrganizationApplication.ENABLED
                                  case "DISABLED" =>
                                      OrganizationApplication.DISABLED
                              }
                          case _ =>
                              throw DeserializationException("Expected status")
                      },
                      elements(1) match {
                          case app: JsObject =>
