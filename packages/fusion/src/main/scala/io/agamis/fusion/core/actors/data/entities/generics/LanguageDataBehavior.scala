...
object LanguageDataBehavior {
  ...
  def apply(state: DataActor.State)(implicit ec: ExecutionContext, wrapper: IgniteClientNodeWrapper): PartialFunction[
    (ActorContext[LanguageDataBehavior.Command], LanguageDataBehavior.Command),
    Behavior[LanguageDataBehavior.Command]
  ] = {
    implicit val store = new LanguageStore
    return {
