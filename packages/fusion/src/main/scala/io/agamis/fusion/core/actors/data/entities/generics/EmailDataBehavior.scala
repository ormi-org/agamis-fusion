...
object EmailDataBehavior {
  ...
  def apply(state: DataActor.State)(implicit ec: ExecutionContext, wrapper: IgniteClientNodeWrapper): PartialFunction[
    (ActorContext[EmailDataBehavior.Command], EmailDataBehavior.Command),
    Behavior[EmailDataBehavior.Command]
  ] = {
    implicit val store = new EmailStore
    return {
