...
class FileSystem(implicit @transient protected val store: FileSystemStore)
    extends Model {
    ...
    @transient
    private var _organizations: List[(Boolean, (Boolean, Organization))] =
        List()
    ...
    def removeOrganization(organization: Organization): FileSystem = {
        _organizations.indexWhere(_._2._2.id == organization.id) match {
            ...
            case index =>
                ...
                if (o._2._1)
                    ...
                    _organizations =
                        _organizations.updated(index, o.copy(_1 = false))
        }
        this
    }
    ...
    def addLicensedApplication(application: Application): FileSystem = {
        _licensedApplications.indexWhere(_.id == application.id) match {
            ...
            case index =>
                _licensedApplications =
                    _licensedApplications.updated(index, application)
        }
        this
    }
    ...
    def persist(implicit
        ec: ExecutionContext
    ): Future[(Transaction, FileSystem)] = {
        this.setUpdatedAt(Timestamp.from(Instant.now()))
        store
            .persistFileSystem(this)
            .transformWith({
