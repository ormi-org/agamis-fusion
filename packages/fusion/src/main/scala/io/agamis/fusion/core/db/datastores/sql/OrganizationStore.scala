  ...
  def persistOrganization(organization: Organization)(implicit ec: ExecutionContext): Future[Unit] = {
    makeTransaction match {
      case Success(tx) =>
        ...
        val fileSystemRelationCache: IgniteCache[String, FilesystemOrganization] = wrapper.getCache[String, FilesystemOrganization](fileSystemStore.cache)
        Future.sequence(
          List(
...
//            )
  ...
  /** A method that deletes several organizations
    *
    * @param organizations organizations to be deleted
  ...
  def bulkDeleteOrganizations(organizations: List[Organization])(implicit ec: ExecutionContext): Future[Unit] = {
    makeTransaction match {
      case Success(tx) =>
        Utils.igniteToScalaFuture(igniteCache.removeAllAsync(organizations.filter({ org => org.relatedProfiles.isEmpty }).map(_.id).toSet.asJava))
          .transformWith({
            case Success(_) =>
