...
import io.agamis.fusion.core.db.models.sql.exceptions.organizations.InvalidOrganizationApplicationStatus
...
class OrganizationApplication private() {
    ...
    protected var _applicationId: UUID = _
    ...
    protected var _licenseFileFsId: UUID = _

    @QuerySqlField(name = "license_file_id", notNull = true)
    protected var _licenseFileId: String = _
}
...
object OrganizationApplication {
    ...
    def apply(organizationId: UUID, applicationId: UUID, status: OrganizationApplication.Status): OrganizationApplication = {
        val relation = new OrganizationApplication()
