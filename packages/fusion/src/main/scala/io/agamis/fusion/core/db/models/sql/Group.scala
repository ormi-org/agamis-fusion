...
class Group(implicit @transient protected val store: GroupStore) extends Model {
    ...
    /** Group name property
      *
      * @return
      *   group name [[java.lang.String String]]
      */
    ...
    /** A method for setting name property
      *
      * @param name
      *   a [[java.lang.String String]] to assign to group name property
      * @return
      *   this object
      */
    ...
    def addMember(profile: Profile): Group = {
        _members.indexWhere(_._2.id == profile.id) match {
            ...
            case index =>
                _members =
                    _members.updated(index, _members(index).copy(_1 = true))
        }
        this
    }
    def removeMember(profile: Profile): Group = {
        _members.indexWhere(_._2.id == profile.id) match {
            ...
            case index =>
                _members =
                    _members.updated(index, _members(index).copy(_1 = false))
        }
        this
    }
    ...
    /** Group property that reflects the organization to which this group is
      * related
      *
      * @return
      *   related
