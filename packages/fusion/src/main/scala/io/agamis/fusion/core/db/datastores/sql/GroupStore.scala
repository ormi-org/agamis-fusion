...
class GroupStore(implicit wrapper: IgniteClientNodeWrapper)
    extends SqlMutableStore[UUID, Group] {

  override val schema: String = "FUSION"
  override val cache: String = s"SQL_${schema}_GROUP"
  override protected val igniteCache: IgniteCache[UUID, Group] =
    if (wrapper.cacheExists(cache)) {
      wrapper.getCache[UUID, Group](cache)
    } else {
      wrapper.createCache[UUID, Group](
        wrapper
          .makeCacheConfig[UUID, Group]
  ...
  /** A factory method that generates a new group object
    *
    * [[Group Group]] is generated along with its '''implicit'''
    * [[GroupStore GroupStore]]
    *
    * @return
    *   a simple [[Group Group]]
    */
  def makeGroup: Group = {
  ...
  def makeGroupsQuery(
    ...
    queryFilters.filters.foreach({ filter =>
      ...
      filter.createdAt match {
        case Some((test, time)) =>
          innerWhereStatement += s"group_created_at ${test match {
            case "eq"  => "="
            case "gt"  => ">"
            case "lt"  => "<"
            case "neq" => "<>"
      ...
      filter.updatedAt match {
        case Some((test, time)) =>
          innerWhereStatement += s"group_updated_at ${test match {
            case "eq"  => "="
            case "gt"  => ">"
            case "lt"  => "<"
            case "neq" => "<>"
  ...
  /** A method that gets several existing groups from database based on provided
    * filters
    *
    * @note
      ...
      case Success(rows) =>
        ...
        val groups = rows
          ...
          .map(entityReflection => {
            val groupedRows =
              getRelationsGroupedRowsFrom(entityReflection, 4, 5)
                      ...
                      case Some(organizationReflections) =>
                        organizationReflections.size match {
                          case 0 =>
                            Future.failed(
                              OrganizationNotFoundException(
                                s"Group ${group.id}:${group.name} might be orphan"
                              )
                            )
                          case 1 =>
                            val organizationReflection =
                              organizationReflections.last
                              ...
                              case result =>
                                result._1.length match {
                                  ...
                                  case _ =>
                                    Future.failed(
                                      DuplicateOrganizationException(
                                        s"Group ${group.id}:${group.name} has duplicate organization relation"
                                      )
                                    )
                                }
                            }
  ...
  /** A method that gets an existing group by its id
    ...
    *   parallelize computing
    * @return
    *   a future [[Group Group]] that corresponds to the provided id
    */
    ...
    getGroups(
      ...
      case Success(groups) =>
        groups.length match {
          case 0 =>
            Future.failed(
              GroupNotFoundException(s"Group $id couldn't be found")
            )
          ...
          .sequence(
            List(
              ...
              profileStore
                ...
                .transformWith({
                  case Success(profiles) =>
                    Future.sequence(profiles.map(_.addGroup(group).persist))
              ...
              profileStore
                ...
                .transformWith({
                  case Success(profiles) =>
                    Future.sequence(profiles.map(_.removeGroup(group).persist))
                  case Failure(cause) => throw GroupNotPersistedException(cause)
                })
            )
          )
                  ...
                  groups
                    .flatMap(gp =>
                      gp.permissions
                        .filter(_._1 == true)
                        .map(relation =>
                          s"${gp.id}:${relation._2.id}"
                        ) zip gp.permissions.map({ perm =>
                        GroupPermission(
                          gp.id,
                          perm._2.id
                        )
                ...
                permissionRelationCache.removeAllAsync(
                  groups
                    .flatMap(gp =>
                      gp.permissions
                        .filter(_._1 == false)
                        .map(relation => s"${gp.id}:${relation._2.id}")
                    )
                    .toSet[String]
                    .asJava
                )
                    ...
                    List(
                      profileStore
                        ...
                        .transformWith({
                          case Success(profiles) =>
                            Future
                              .sequence(profiles.map(_.addGroup(group).persist))
                          case Failure(cause) =>
                            throw GroupNotPersistedException(cause)
                        }),
                      profileStore
                        .getProfiles(
                          ProfilesFilters(
                        ...
                        .transformWith({
                          case Success(profiles) =>
                            Future.sequence(
                              profiles.map(_.removeGroup(group).persist)
                            )
                          case Failure(cause) =>
                            throw GroupNotPersistedException(cause)
                        })
            ...
            List(
              ...
              profileStore
                ...
                .transformWith({
                  case Success(profiles) =>
                    profileStore
                      .bulkPersistProfiles(profiles.map(_.removeGroup(group)))
                  case Failure(cause) => Future.failed(cause)
                }),
                  ...
                  group.relatedOrganization match {
                    ...
                    case None =>
                      throw OrganizationNotFoundException(
                        "Specified group is organization orphan"
                      )
                  }
                )
                .transformWith({
      ...
      case Success(tx) =>
        ...
        Future
          .sequence(
            List(
              Utils.igniteToScalaFuture(
                igniteCache.removeAllAsync(groups.map(_.id).toSet.asJava)
              ),
              // Delete relations
              // Delete permission relations
              Utils.igniteToScalaFuture(
              ...
              groups.map(g =>
                profileStore
                  ...
                  .transformWith({
                    case Success(profiles) =>
                      profileStore
                        .bulkPersistProfiles(profiles.map(_.removeGroup(g)))
                    case Failure(cause) => Future.failed(cause)
                  })
              )
            } ++ {
              ...
              List(
                Future
                  .sequence(
                    groups.foldLeft(List[Future[Organization]]())((acc, g) =>
                                ...
                                .copy(filters =
                                  List(
                                    OrganizationStore
                                      ...
                                      .copy(id = List(organization.id.toString))
                                  )
                                )
                            )
                            .transformWith({
                  ...
                  .transformWith({
                    case Success(organizations) =>
                      organizationStore.bulkPersistOrganizations(organizations)
                    case Failure(cause) => Future.failed(cause)
                  })
              )
            }
