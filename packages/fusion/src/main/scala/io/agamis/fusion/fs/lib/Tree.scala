...
import io.agamis.fusion.fs.lib.exceptions.tree.{NotADirectoryException, PathParsingException, UnhandledException, UnknownException}
...
class Tree private (fs: FileSystem) {
  ...
  /** A method for creating a new file in the '''File Tree'''
    ...
    * @return a future boolean which attests the actual creation of the file
    */

  def createFile(file: File)(implicit ec: ExecutionContext, wrapper: ReactiveMongoWrapper, igniteWrapper: IgniteClientNodeWrapper): Future[Boolean] = {
    file.path match {
      case Some(path: String) =>
        getFileFromPath(path).transformWith({
          case Success(_) => Future.failed(FileAlreadyExistsException())
          case Failure(cause) =>
            cause match {
              ...
              case cause: Throwable => Future.failed(UnhandledException(cause))
  ...
  /** A method for getting an existing file in the '''File Tree''' by its '''id'''
    ...
    */
  def getFileFromId(id: String)(implicit ec: ExecutionContext, wrapper: ReactiveMongoWrapper, igniteWrapper: IgniteClientNodeWrapper): Future[File] = {
    new CachedFileStore()
  ...
  /** A method for getting several existing files in the '''File Tree''' by their '''id'''
    ...
    */
  def getManyFiles(ids: String*)(implicit ec: ExecutionContext, wrapper: ReactiveMongoWrapper, igniteWrapper: IgniteClientNodeWrapper): Future[List[File]] = {
    new CachedFileStore()
      .getMany(ids.toList).transformWith({
      case Success(files) =>
        val cacheRetrievedFiles: List[File] = files.asScala.values.toList
  ...
  /** A method for getting parent file in the '''File Tree''' by one of its '''child''' aka "file"
    ...
    * @return a future file that reflects actual file state in the datastore
    */
  def getParentOf(file: File)(implicit ec: ExecutionContext, wrapper: ReactiveMongoWrapper): Future[File] = {
    file.path match {
      case Some(path: String) =>
  ...
  /** A method for deleting a file in the '''File Tree'''
    ...
    */
  def deleteFile(file: File)(implicit ec: ExecutionContext, wrapper: ReactiveMongoWrapper, igniteWrapper: IgniteClientNodeWrapper): Future[File] = {
    getChildrenOf(file).transformWith({
      case Success(children) =>
        // Handle case where file to delete has child
        if (children.nonEmpty) return Future.failed(new Exception("Directory can't be deleted if not empty"))
  ...
  /** A method for deleting many files in the '''File Tree'''
    ...
    *     })
    * }}}
    * @param files         repeatable parameters which consist of files to delete
    * @param ec            the '''implicit''' ExecutionContext used to parallelize computing
    * @param wrapper       the '''implicit''' MongoDB wrapper used to read data
    * @param igniteWrapper the '''implicit''' Apache Ignite wrapper used to write cache
    * @return a future result which contains count of deleted files and caught errors
    */
  def deleteManyFiles(files: File*)(implicit ec: ExecutionContext, wrapper: ReactiveMongoWrapper, igniteWrapper: IgniteClientNodeWrapper): Future[DeleteManyFilesResult] = {
