...
import io.agamis.fusion.core.db.datastores.sql.exceptions.typed.permissions.{
...
class PermissionStore(implicit wrapper: IgniteClientNodeWrapper)
    extends SqlMutableStore[UUID, Permission] {

  override val schema: String = "FUSION"
  override val cache: String = s"SQL_${schema}_PERMISSION"
  override protected val igniteCache: IgniteCache[UUID, Permission] =
    if (wrapper.cacheExists(cache)) {
      wrapper.getCache[UUID, Permission](cache)
    } else {
      wrapper.createCache[UUID, Permission](
        wrapper
          .makeCacheConfig[UUID, Permission]
  ...
  def makePermissionQuery(
      queryFilters: PermissionStore.GetPermissionsFilters
  ): SqlStoreQuery = {
    ...
    queryFilters.filters.foreach({ filter =>
      ...
      if (filter.id.nonEmpty) {
        innerWhereStatement += s"permission_id in (${(for (_ <- 1 to filter.id.length)
          yield "?").mkString(",")})"
        queryArgs ++= filter.id
      }
      // manage keys search
      if (filter.key.nonEmpty) {
        innerWhereStatement += s"permission_key in (${(for (_ <- 1 to filter.key.length)
          yield "?").mkString(",")})"
      ...
      filter.createdAt match {
        case Some((test, time)) =>
          innerWhereStatement += s"permission_created_at ${test match {
            case "eq"  => "="
            case "gt"  => ">"
            case "lt"  => "<"
            case "neq" => "<>"
      ...
      filter.updatedAt match {
        case Some((test, time)) =>
          innerWhereStatement += s"permission_updated_at ${test match {
            case "eq"  => "="
            case "gt"  => ">"
            case "lt"  => "<"
            case "neq" => "<>"
  ...
  def getPermissions(
    ...
    executeQuery(
                ...
                (for (
                  permission <- Right(
                    makePermission
                      ...
                      .setCreatedAt(
                        Utils.timestampFromString(permissionDef(5)) match {
                          case createdAt: Timestamp => createdAt
                          case _                    => null
                        }
                      )
                      .setUpdatedAt(
                        Utils.timestampFromString(permissionDef(6)) match {
                    ...
                    permissionReflections.head.partition(
                                ...
                                application <- Right(
                                  ...
                                  try {
                                    application.setStatus(
                                      Application.Status
                                        .fromInt(
                                          applicationDef(4).asInstanceOf[Int]
                                        )
                                        .get
                                    )
                                  } catch {
                                    case e: InvalidApplicationStatus =>
                                      Future.failed(
                                        new PermissionQueryExecutionException(e)
                                      )
                          ...
                          for (
                            profile <- Right(
                              new ProfileStore().makeProfile
                                ...
                                .setLastLogin(
                                  Utils
                                    .timestampFromString(profileDef(4)) match {
                                ...
                                .setCreatedAt(
                                  Utils
                                    .timestampFromString(profileDef(6)) match {
                                    case createdAt: Timestamp => createdAt
                                    case _                    => null
                                  }
                                )
                                .setUpdatedAt(
                                  Utils
                                    .timestampFromString(profileDef(7)) match {
                                    case updatedAt: Timestamp => updatedAt
                                    case _                    => null
                                  }
                                )
                    ...
                    groupedRows.get("GROUP") match {
                      case Some(groupReflections) =>
                        groupReflections.foreach({ groupReflection =>
                          ...
                          for (
                            group <- Right(
                              new GroupStore().makeGroup
                                .setId(groupDef(0))
                                .setName(groupDef(1))
                                .setCreatedAt(
                                  Utils.timestampFromString(groupDef(2)) match {
                                    case createdAt: Timestamp => createdAt
                                    case _                    => null
                                  }
                                )
                                .setUpdatedAt(
                                  Utils.timestampFromString(groupDef(3)) match {
                                    case updatedAt: Timestamp => updatedAt
                                    case _                    => null
                                  }
                                )
      ...
      case Failure(cause) =>
        Future.failed(PermissionQueryExecutionException(cause))
    })
  }

  def getAllPermissions(implicit
      ec: ExecutionContext
  ...
  def getPermissionById(
      id: String
  )(implicit ec: ExecutionContext): Future[Permission] = {
    getPermissions(
      PermissionStore
        .GetPermissionsFilters()
        .copy(
      ...
      case Success(permissions) =>
        permissions.length match {
          case 0 =>
            Future.failed(
              PermissionNotFoundException(s"Permission $id couldn't be found")
            )
    ...
    getPermissions(
      ...
      case Success(permissions) =>
        permissions.length match {
          case 0 =>
            Future.failed(
              PermissionNotFoundException(s"Permission $key couldn't be found")
            )
          case 1 => Future.successful(permissions.head)
          case _ => Future.failed(new DuplicatePermissionException)
        }
  ...
  def persistPermission(permission: Permission)(implicit ec: ExecutionContext): Future[Transaction] = {
          ...
          .sequence(
            List(
              Utils.igniteToScalaFuture(
                igniteCache.putAsync(permission.id, permission)
              )
            ) ++ {
                    ...
                    .transformWith({
                      case Success(profiles) =>
                        profileStore.bulkPersistProfiles(
                          (permission.owningProfiles
                            .sortBy(_._2.id)
                            .map(_._1) zip profiles).map({ profile =>
                            if (profile._1) {
                    ...
                    .transformWith({
                      case Success(groups) =>
                        groupStore.bulkPersistGroups(
                          (permission.owningGroups
                            .sortBy(_._2.id)
                            .map(_._1) zip groups).map({ group =>
                            if (group._1) {
            ...
            List(
              Utils.igniteToScalaFuture(
                igniteCache.putAllAsync(
                  (permissions
                    .map(_.id) zip permissions).toMap[UUID, Permission].asJava
                )
              ),
              profileStore
                .getProfiles(
  ...
  def deletePermission(permission: Permission)(implicit ec: ExecutionContext): Future[Transaction] = {
    makeTransaction match {
      case Success(tx) =>
        val profileStore = new ProfileStore()
        val groupStore = new GroupStore()
