package io.agamis.fusion.external.api.rest.dto.fs.file.metadata
...
import io.agamis.fusion.external.api.rest.dto.common.JsonFormatters.InstantFormat
import io.agamis.fusion.external.api.rest.dto.fs.file.metadata.meta.FusionXmlMetaDto
import io.agamis.fusion.external.api.rest.dto.fs.file.metadata.meta.FusionXmlMetaDtoJsonProtocol._
import spray.json._
  ...
  implicit def apply(dto: MetadataDto): Metadata = {
    Metadata(
      ...
      dto.lastVersionDate.orNull match {
        case lastVersionDate: String => Some(Instant.parse(lastVersionDate))
        case null => None
      },
      ...
      dto.fusionXml.orNull match {
        case fXmlMetaDto: FusionXmlMetaDto => Some(FusionXmlMetaDto(fXmlMetaDto))
        case null => None
      },
      dto.hidden,
      dto.readonly
...
object MetadataDtoJsonProtocol extends DefaultJsonProtocol {
  ...
  implicit val metadataFormat: RootJsonFormat[MetadataDto] = jsonFormat9(MetadataDto.apply)
}