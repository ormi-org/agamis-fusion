...
object OrganizationTypeDataBehavior {
  ...
  def apply(state: DataActor.State)(implicit ec: ExecutionContext, wrapper: IgniteClientNodeWrapper): PartialFunction[
    (ActorContext[OrganizationTypeDataBehavior.Command], OrganizationTypeDataBehavior.Command),
    Behavior[OrganizationTypeDataBehavior.Command]
  ] = {
    implicit val store = new OrganizationTypeStore
    return {
