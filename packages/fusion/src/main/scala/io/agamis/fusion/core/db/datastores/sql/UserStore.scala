...
class UserStore(implicit wrapper: IgniteClientNodeWrapper)
    ...
    def makeUsersQuery(
        queryParams: UserStore.UserQueryParams
    ): SqlStoreQuery = {
        ...
        queryParams.filters.foreach({ filter =>
            ...
            if (filter.username.nonEmpty) {
                innerWhereStatement += filter.username
                    .map({ f =>
                        queryArgs += f._2
            ...
            if (filter.profile_id.nonEmpty) {
                innerWhereStatement += filter.profile_id
                    .map({ profile_id =>
            ...
            if (filter.profile_alias.nonEmpty) {
                innerWhereStatement += filter.profile_alias
                    .map({ profile_alias =>
            ...
            if (filter.profile_lastname.nonEmpty) {
                innerWhereStatement += filter.profile_lastname
                    .map({ profile_lastname =>
            ...
            if (filter.profile_firstname.nonEmpty) {
                innerWhereStatement += filter.profile_firstname
                    .map({ profile_firstname =>
                        queryArgs += profile_firstname
                        s"${EntityQueryParams.IN_WHERE_CLAUSE(UserStore.Column.PROFILE.FIRSTNAME().name)} LIKE \"%?%\""
                    })
                    .mkString(" OR ")
            }
            filter.profile_lastLogin.map({
                _ match {
                    case (test, time) =>
                        innerWhereStatement += s"${EntityQueryParams.IN_WHERE_CLAUSE(
                              UserStore.Column.PROFILE.LAST_LOGIN().name
                            )} ${test match {
                                ...
                                case _ =>
                                    throw InvalidComparisonOperatorException(test)
                            }} ?"
                        queryArgs += time.toString
                }
            })
            filter.profile_createdAt.foreach({
                _ match {
                    case (test, time) =>
                        innerWhereStatement += s"${EntityQueryParams.IN_WHERE_CLAUSE(
                              UserStore.Column.PROFILE.CREATED_AT().name
                            )} ${test match {
                                case Filter.ComparisonOperator.Equal =>
                                    Filter.ComparisonOperator.SQL.Equal
                                case Filter.ComparisonOperator.GreaterThan =>
            ...
            filter.profile_updatedAt.foreach({
                _ match {
                    case (test, time) =>
                        innerWhereStatement += s"${EntityQueryParams.IN_WHERE_CLAUSE(
                              UserStore.Column.PROFILE.UPDATED_AT().name
                            )} ${test match {
                                case Filter.ComparisonOperator.Equal =>
                                    Filter.ComparisonOperator.SQL.Equal
                                case Filter.ComparisonOperator.GreaterThan =>
            ...
            if (innerWhereStatement.nonEmpty)
                  ...
                  s" ORDER BY ${queryParams.orderBy
                          .map(o =>
                              s"u.${o._1.name} ${o._2 match {
                                      ...
                                      case _ =>
                                          throw InvalidOrderingOperatorException(o._2)
                                  }}"
                          )
                          .mkString(", ")}"
              case false => ""
        ...
        baseQueryString = baseQueryString.replace(
          ...
          queryParams.pagination match {
              ...
              case None =>
                  s" LIMIT ${Pagination.Default.Limit} OFFSET ${Pagination.Default.Offset} "
          }
        )
        ...
        baseQueryString = (queryParams.include match {
            ...
            case Some(fields) =>
                UserStore.Includables.values.toList.foldLeft(baseQueryString)(
                  (acc, incl) => {
                      acc.replace(
                        incl.placeholder,
                        if (fields.contains(incl.name)) "TRUE" else "FALSE"
                      )
                  }
                )
        })
    ...
    def getUsers(
        ...
        executeQuery(makeUsersQuery(queryParams)).transformWith({
                ...
                val users = rows.map({ row =>
                    (for (
                      user <- Right(
                        ...
                        makeUser
                            .setId(
                              row(UserStore.Column.ID().order)
                                  .asInstanceOf[UUID]
                            )
                            .setUsername(
                              row(UserStore.Column.USERNAME().order)
                                  .asInstanceOf[String]
                            )
                            .setPasswordHash(
                              row(UserStore.Column.PASSWORD().order)
                                  .asInstanceOf[String]
                            )
                            .setCreatedAt(
                              row(UserStore.Column.CREATED_AT().order)
                                  .asInstanceOf[Timestamp] match {
                                  case ts: Timestamp => ts
                                  case _             => null
                              }
                            )
                            .setUpdatedAt(
                              row(UserStore.Column.UPDATED_AT().order)
                                  .asInstanceOf[Timestamp] match {
                                  case ts: Timestamp => ts
                                  case _             => null
                              }
                          ...
                          // map profiles
                          row(UserStore.Column.PROFILES().order)
                              .asInstanceOf[String]
                              .split("""\|\|""")
                              .foreach({ profileString =>
                                  for (
                                    profileReflection <- Right(
                                      profileString.split("""\|>\|""")
                                    );
                                    profile <- Right(
                                        ...
                                        Try(
                                          profileReflection(
                                            ...
                                            case Success(isActive) =>
                                                if (isActive)
                                                    Right(profile.setActive())
                                                else
                                                    Right(profile.setInactive())
            ...
            case Failure(cause) =>
                Future.failed(UserQueryExecutionException(cause))
        })
    }
        ...
        getUsers(
          UserStore
            ...
            case Success(users) =>
                users.length match {
                    case 0 =>
                        Future.failed(NoEntryException("User table is empty"))
    ...
    /** Method used to get a single User entity from database based on its id
      * attribute
      *
      * @param id
      *   the id selector
      ...
      * @return
      *   a future [[User User]] which reflects user state fetched from database
      */
    def getUserById(id: String)(implicit ec: ExecutionContext): Future[User] = {
        Utils
            .igniteToScalaFuture(igniteCache.getAsync(UUID.fromString(id)))
            .transformWith({
                case Success(user) =>
                    user match {
            ...
            case Success(users) =>
                users.length match {
                    case 0 =>
                        Future.failed(
                          UserNotFoundException(
                            s"User $username couldn't be found"
                          )
                        )
    ...
    def persistUser(
        user: User
    )(implicit ec: ExecutionContext): Future[(Transaction, User)] = {
    ...
    /** A result of bulkPersistUsers method
      *
      * @constructor
      *   create a new BulkPersistUsersResult with a count of inserted Users and
      *   a list of errors
      * @param inserts
                        ...
                        .sequence(
                          users.map(user =>
                              Utils.igniteToScalaFuture(
                                igniteCache.containsKeyAsync(user.id)
                              )
                          )
                        )
                            ...
                            Future.successful(
                              BulkPersistUsersResult(
                                ...
                                lookup.get
                                    .filter(_._2 == false)
                                    .map(
                                      "Insert user " + _._1.toString + " failed"
                                    )
                              )
                            )
                        })
                            ...
                            Future.successful(
                              BulkDeleteUsersResult(
                                ...
                                lookup.get
                                    .filter(_._2 == true)
                                    .map(
                                      "Failed to delete user " + _._1.toString
                                    )
                              )
                            )
                        })
...
object UserStore {
    case class UsersFilter(
        ...
        profile_lastLogin: List[(String, Timestamp)] =
            List(), // (date, (eq, lt, gt, ne))
        profile_createdAt: List[(String, Timestamp)] =
            List(), // (date, (eq, lt, gt, ne))
        profile_updatedAt: List[(String, Timestamp)] =
            List(), // (date, (eq, lt, gt, ne))
        createdAt: List[(String, Timestamp)] =
            List(), // (date, (eq, lt, gt, ne))
        updatedAt: List[(String, Timestamp)] =
            List() // (date, (eq, lt, gt, ne))
    )
    ...
    object Column {
        ...
        case class CREATED_AT(
            val order: Int = 3,
            val name: String = "u.CREATED_AT"
        ) extends EntityQueryParams.Column
        case class UPDATED_AT(
            val order: Int = 4,
            val name: String = "u.UPDATED_AT"
        ) extends EntityQueryParams.Column
        ...
        object PROFILE {
            ...
            case class LASTNAME(
                val order: Int = 2,
                val name: String = "p.LASTNAME"
            ) extends EntityQueryParams.Column
            case class FIRSTNAME(
                val order: Int = 3,
                val name: String = "p.FIRSTNAME"
            ) extends EntityQueryParams.Column
            case class LAST_LOGIN(
                val order: Int = 4,
                val name: String = "p.LAST_LOGIN"
            ) extends EntityQueryParams.Column
            case class IS_ACTIVE(
                val order: Int = 5,
                val name: String = "p.IS_ACTIVE"
            ) extends EntityQueryParams.Column
            case class CREATED_AT(
                val order: Int = 6,
