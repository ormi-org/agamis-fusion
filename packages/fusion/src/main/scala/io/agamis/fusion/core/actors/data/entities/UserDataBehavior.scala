...
object UserDataBehavior {
    ...
    sealed trait Response extends DataActorResponse with DataActor.State
    ...
    @JsonSubTypes(
      Array(
        ...
        new JsonSubTypes.Type(
          value = classOf[InternalException],
            ...
            case (ctx: ActorContext[Command], qry: GetUserByUsername) => {
                if (
                  state match {
                    ...
                    ctx.pipeToSelf(
                      store.getUserByUsername(qry.username.toString())
                    ) {
                        case Success(u) =>
                            val newState = SingleUserState(
                              state.entityId,
                ...
                if (
                  crt.userMutation.username.isEmpty || crt.userMutation.password.isEmpty
                    ...
                    crt.replyTo ! SingleUserState(
                      ...
                      None,
                      InternalException(
                        "Missing field among: username, password"
                      )
                    )
                } else {
                    val mustCache: Boolean =
