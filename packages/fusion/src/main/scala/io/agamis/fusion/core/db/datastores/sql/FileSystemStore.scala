...
import io.agamis.fusion.core.db.datastores.sql.exceptions.typed.filesystems.{DuplicateFilesystemException, FilesystemNotFoundException, FilesystemNotPersistedException, FilesystemQueryExecutionException}
...
class FileSystemStore(implicit wrapper: IgniteClientNodeWrapper) extends SqlMutableStore[UUID, FileSystem] {
  ...
  override protected val igniteCache: IgniteCache[UUID, FileSystem] = if (wrapper.cacheExists(cache)) {
  ...
  def makeFileSystemsQuery(queryFilters: FileSystemStore.GetFileSystemsFilters): SqlStoreQuery = {
    ...
    queryFilters.filters.foreach({ filter =>
      ...
      if (filter.id.nonEmpty) {
        innerWhereStatement += s"fs_id in (${(for (_ <- 1 to filter.id.length) yield "?").mkString(",")})"
        queryArgs ++= filter.id
      }
      // manage rootdirIds search
      if (filter.rootdirId.nonEmpty) {
        innerWhereStatement += s"fs_rootdir_id in (${(for (_ <- 1 to filter.rootdirId.length) yield "?").mkString(",")})"
        queryArgs ++= filter.rootdirId
      }
      // manage labels search
      if (filter.label.nonEmpty) {
        innerWhereStatement += s"fs_label in (${(for (_ <- 1 to filter.label.length) yield "?").mkString(",")})"
  ...
  def getFileSystems(queryFilters: FileSystemStore.GetFileSystemsFilters)(implicit ec: ExecutionContext): Future[List[FileSystem]] = {
    executeQuery(makeFileSystemsQuery(queryFilters)).transformWith({
      case Success(rows) =>
        ...
        val fileSystems = rows.map(_.head).distinct.map(entityReflections(_)).map(entityReflection => {
  ...
  def getFileSystemById(id: String)(implicit ec: ExecutionContext): Future[FileSystem] = {
    getFileSystems(
      ...
      case Success(fileSystems) =>
        fileSystems.length match {
          case 0 => Future.failed(FilesystemNotFoundException(s"FileSystem $id couldn't be found"))
          ...
          Future.sequence(
            List(
              ...
              Utils.igniteToScalaFuture(relationCache.removeAllAsync(
                fileSystem.organizations
                  .filter(_._1 == false)
                  .map(relation => s"${fileSystem.id}:${relation._2._2.id}").toSet[String].asJava
              )),
              // Save entity
          ...
          case Success(tx) =>
            ...
            Future.sequence(
              List(
                ...
                Utils.igniteToScalaFuture(relationCache.putAllAsync(
                  fileSystems.flatMap(fs => fs.organizations.filter(_._1 == true).map(relation => s"${fs.id}:${relation._2._2.id}") zip fs.organizations.map({ org =>
                    FilesystemOrganization(
                      fs.id,
                      org._2._2.id,
                      org._1
                ...
                Utils.igniteToScalaFuture(relationCache.removeAllAsync(
                  fileSystems.flatMap(fs =>
                    fs.organizations.filter(_._1 == false).map(relation => s"${fs.id}:${relation._2._2.id}")).toSet[String].asJava
                ))
              )
            ).transformWith({
              case Success(_) =>
  ...
  /** A method that deletes several fileSystems
    *
    * @param fileSystems fileSystems to be deleted
    * @param ec          the '''implicit''' [[ExecutionContext ExecutionContext]] used to parallelize computing
    * @return a future confirmation of state change
    */
  def bulkDeleteFileSystems(fileSystems: List[FileSystem])(implicit ec: ExecutionContext): Future[Unit] = {
    fileSystems.find({ fs => fs.organizations.nonEmpty }) match {
      ...
      case None =>
        Utils.igniteToScalaFuture(igniteCache.removeAllAsync(fileSystems.map(_.id).toSet.asJava))
          .transformWith({
            case Success(_) => Future.unit
            case Failure(cause) => Future.failed(FilesystemNotPersistedException(cause))
          })
