...
class Permission(implicit @transient protected val store: PermissionStore)
    extends Model {
    ...
    /** A mapping of all labels for this Permission
      *
      * @return
      *   a map of [(text_id, language_id), (language_code, content)]
      */
    ...
    def label(languageCode: String): String = {
        _labels.find(_._2._1 == languageCode) match {
            ...
            case None =>
                throw new Exception(
                  "Text not found for this languageCode"
                ) // TODO : custom
        }
    }
    def label(languageId: UUID): String = {
        _labels.get((_labelTextId, languageId)) match {
            ...
            case None =>
                throw new Exception(
                  "Text not found for this languageId"
                ) // TODO : custom
        }
    }
    def setLabel(language: Language, content: String): Permission = {
        _labels =
            _labels.+(((_labelTextId, language.id), (language.code, content)))
        this
    }
    ...
    def setDescription(language: Language, content: String): Permission = {
        _descriptions = _descriptions.+(
          ((_descriptionTextId, language.id), (language.code, content))
        )
        this
    }
    ...
    def addOwningGroup(group: Group): Permission = {
        _owningGroups.indexWhere(_._2.id == group.id) match {
            ...
            case index =>
                _owningGroups = _owningGroups.updated(
                  index,
                  _owningGroups(index).copy(_1 = true)
                )
        }
        this
    }
    ...
    def removeOwningGroup(group: Group): Permission = {
        _owningGroups.indexWhere(_._2.id == group.id) match {
            ...
            case index =>
                _owningGroups = _owningGroups.updated(
                  index,
                  _owningGroups(index).copy(_1 = false)
                )
        }
        this
    }
    ...
    def addOwningProfile(profile: Profile): Permission = {
        _owningProfiles.indexWhere(_._2.id == profile.id) match {
            ...
            case index =>
                _owningProfiles = _owningProfiles.updated(
                  index,
                  _owningProfiles(index).copy(_1 = true)
                )
        }
        this
    }
    ...
    def removeOwningProfile(profile: Profile): Permission = {
        _owningProfiles.indexWhere(_._2.id == profile.id) match {
            ...
            case index =>
                _owningProfiles = _owningProfiles.updated(
                  index,
                  _owningProfiles(index).copy(_1 = false)
                )
        }
        this
    }
    ...
    def persist(implicit
        ec: ExecutionContext
    ): Future[(Transaction, Permission)] = {
        this.setUpdatedAt(Timestamp.from(Instant.now()))
        this.store
            .persistPermission(this)
            .transformWith({
