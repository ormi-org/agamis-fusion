...
const profilesRoutes = (server: Server) => {[
    server.get(`/api/v1/organizations/${DEFAULT_ORGANIZATION_ID}/profiles`, (schema, request) => {
        const {
            offset,
            limit,
            order_by,
        ...
        const excluded = EXCLUDED_FIELDS.filter(f => !includedFields.includes(f));
        return profiles.org_profiles_sample_with_user
                .sort((a, b) => a[order[0]] - b[order[0]]*Number.parseInt(order[1]))
