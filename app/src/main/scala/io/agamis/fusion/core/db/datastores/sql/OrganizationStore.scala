...
class OrganizationStore(implicit wrapper: IgniteClientNodeWrapper)
  ...
  override protected val igniteCache: IgniteCache[UUID, Organization] =
  ...
  def makeOrganizationsQuery(
      queryFilters: OrganizationStore.GetOrganizationsFilters
  ): SqlStoreQuery = {
    ...
    queryFilters.filters.foreach({ filter =>
      ...
      if (filter.id.nonEmpty) {
        innerWhereStatement += s"org_id in (${(for (_ <- 1 to filter.id.length)
          yield "?").mkString(",")})"
        queryArgs ++= filter.id
      }
      // manage labels search
      if (filter.label.nonEmpty) {
        innerWhereStatement += s"org_label in (${(for (_ <- 1 to filter.label.length)
          yield "?").mkString(",")})"
        queryArgs ++= filter.label
      }
      // manage types search
      if (filter.oType.nonEmpty) {
        innerWhereStatement += s"org_type in (${(for (_ <- 1 to filter.oType.length)
          yield "?").mkString(",")})"
      ...
      filter.createdAt match {
        case Some((test, time)) =>
          innerWhereStatement += s"org_created_at ${test match {
            case "eq"  => "="
            case "gt"  => ">"
            case "lt"  => "<"
            case "neq" => "<>"
      ...
      filter.updatedAt match {
        case Some((test, time)) =>
          innerWhereStatement += s"org_updated_at ${test match {
            case "eq"  => "="
            case "gt"  => ">"
            case "lt"  => "<"
            case "neq" => "<>"
                      ...
                      case result =>
                        result._1.length match {
                          case 0 =>
                            Future.failed(
                              OrganizationtypeNotFoundException(
                                s"Organization type of organization ${organization.id}:${organization.label} might be undefined"
                              )
                            )
                          case 1 =>
                            val orgTypeDef = result._1(0)._2
                            for (
                              organizationType <- Right(
                                ...
                                result._2.filter(
                                  _._1 == "ORGTYPE_LABEL_LANG_VARIANT"
                                ) match {
                                  case result =>
                                    if (result.isEmpty) {
                                      Future.failed(
                                        TextNotFoundException(
                                          s"Organization type ${organizationType.id} might lack of label in any language"
                                        )
                                      )
                                    } else {
                                      result.foreach({ orgTypeLangVariant =>
                                        val orgTypeLangVariantDef =
                                          orgTypeLangVariant._2
                          ...
                          case _ =>
                            Future.failed(
                              DuplicateOrganizationtypeException(
                                s"Organization ${organization.id}:${organization.label} has duplicate organization types"
                              )
                            )
                        }
                    }
                          ...
                          for (
                            profile <- Right(
                              new ProfileStore().makeProfile
                                ...
                                .setCreatedAt(
                                  Utils
                                    .timestampFromString(profileDef(8)) match {
                                    case createdAt: Timestamp => createdAt
                                    case _                    => null
                                  }
                                )
                                .setUpdatedAt(
                                  Utils
                                    .timestampFromString(profileDef(9)) match {
                                    case updatedAt: Timestamp => updatedAt
                                    case _                    => null
                                  }
                                )
                          ...
                          for (
                            fileSystem <- Right(
                              new FileSystemStore().makeFileSystem
                                ...
                                .setLabel(fsDef(2))
                                .setCreatedAt(
                                  Utils.timestampFromString(fsDef(5)) match {
                                    case createdAt: Timestamp => createdAt
                                    case _                    => null
                                  }
                                )
                                .setUpdatedAt(
                                  Utils.timestampFromString(fsDef(6)) match {
                                    case updatedAt: Timestamp => updatedAt
                                    case _                    => null
                                  }
                                )
  ...
  def getAllOrganizations(implicit
      ec: ExecutionContext
  ...
  def getOrganizationById(
      id: String
  )(implicit ec: ExecutionContext): Future[Organization] = {
    getOrganizations(
      OrganizationStore
        .GetOrganizationsFilters()
        .copy(
      ...
      case Success(organizations) =>
        organizations.length match {
          case 0 =>
            Future.failed(
              OrganizationNotFoundException(
                s"Organization $id couldn't be found"
              )
            )
...
//            )
    ...
    Utils
      .igniteToScalaFuture(
        igniteCache.putAllAsync(
          (organizations
            .map(_.id) zip organizations).toMap[UUID, Organization].asJava
        )
      )
      .transformWith({
        case Success(_) => Future.unit
  ...
  def deleteOrganization(
    ...
    if (organization.relatedProfiles.nonEmpty)
      return Future.failed(
        OrganizationNotPersistedException(
          "Organization still contains profiles and can't be deleted"
        )
      )
    if (organization.relatedGroups.nonEmpty)
      return Future.failed(
        OrganizationNotPersistedException(
          "Organization still contains groups and can't be deleted"
        )
      )
    Utils
      .igniteToScalaFuture(igniteCache.removeAsync(organization.id))
      .transformWith({
        case Success(_) => Future.unit
        case Failure(cause) =>
          Future.failed(OrganizationNotPersistedException(cause))
        ...
        Utils
          .igniteToScalaFuture(
            igniteCache.removeAllAsync(
              organizations
                .filter({ org => org.relatedProfiles.isEmpty })
                .map(_.id)
                .toSet
                .asJava
            )
