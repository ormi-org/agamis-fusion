...
object FileSystemDataBehavior {
  ...
  def apply(state: DataActor.State)(implicit ec: ExecutionContext, wrapper: IgniteClientNodeWrapper): PartialFunction[
    (ActorContext[FileSystemDataBehavior.Command], FileSystemDataBehavior.Command),
    Behavior[FileSystemDataBehavior.Command]
  ] = {
    implicit val store = new FileSystemStore
    return {
