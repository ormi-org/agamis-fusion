...
import io.agamis.fusion.core.db.datastores.sql.exceptions.typed.applications.{
...
/** A class to manage and reflects [[Application Application]] life-cycle in the
  * SQL database
  *
  * @param wrapper
  ...
  */
class ApplicationStore(implicit wrapper: IgniteClientNodeWrapper)
    extends SqlMutableStore[UUID, Application] {

  override val schema: String = "FUSION"
  override val cache: String = s"SQL_${schema}_APPLICATION"
  override protected val igniteCache: IgniteCache[UUID, Application] =
    if (wrapper.cacheExists(cache)) {
      wrapper.getCache[UUID, Application](cache)
    } else {
      wrapper.createCache[UUID, Application](
        wrapper
          .makeCacheConfig[UUID, Application]
  ...
  /** A factory method that generates a new application object
    *
    * [[Application Application]] is generated along with its '''implicit'''
    * [[ApplicationStore ApplicationStore]]
    *
    * @return
    *   a simple [[Application Application]]
    */
  ...
  /** A factory method that generates an SQL query based on provided filters
    ...
    */
  def makeApplicationQuery(
      queryFilters: ApplicationStore.GetApplicationsFilters
  ): SqlStoreQuery = {
    ...
    queryFilters.filters.foreach({ filter =>
      ...
      if (filter.id.nonEmpty) {
        innerWhereStatement += s"app_id in (${(for (_ <- 1 to filter.id.length)
          yield "?").mkString(",")})"
        queryArgs ++= filter.id
      }
      // manage appUniversalId search
      if (filter.appUniversalId.nonEmpty) {
        innerWhereStatement += s"app_universal_id in (${(for (_ <- 1 to filter.id.length)
          yield "?").mkString(",")})"
        queryArgs ++= filter.appUniversalId
      }
      // manage status search
      if (filter.status.nonEmpty) {
        innerWhereStatement += s"app_status in (${(for (_ <- 1 to filter.id.length)
          yield "?").mkString(",")})"
      ...
      if (filter.manifestUrl.nonEmpty) {
        innerWhereStatement += s"app_manifest_url in (${(for (_ <- 1 to filter.id.length)
          yield "?").mkString(",")})"
        queryArgs ++= filter.manifestUrl
      }
      // manage storeUrl search
      if (filter.storeUrl.nonEmpty) {
        innerWhereStatement += s"app_store_url in (${(for (_ <- 1 to filter.id.length)
          yield "?").mkString(",")})"
      ...
      filter.createdAt match {
        case Some((test, time)) =>
          innerWhereStatement += s"app_created_at ${test match {
            case "eq"  => "="
            case "gt"  => ">"
            case "lt"  => "<"
            case "neq" => "<>"
      ...
      filter.updatedAt match {
        case Some((test, time)) =>
          innerWhereStatement += s"app_updated_at ${test match {
            case "eq"  => "="
            case "gt"  => ">"
            case "lt"  => "<"
            case "neq" => "<>"
  ...
  /** A method that gets several existing applications from database based on
    * provided filters
    *
    * @note
    ...
    * @return
    *   a future [[List List]] of [[Application Application]]
    */
  def getApplications(
      queryFilters: ApplicationStore.GetApplicationsFilters
  )(implicit ec: ExecutionContext): Future[List[Application]] = {
    executeQuery(makeApplicationQuery(queryFilters)).transformWith({
      case Success(rows) =>
        val entityReflections = rows.groupBy(_.head)
        val applications = rows
          .map(_.head)
          .distinct
          .map(entityReflections(_))
          .map(entityReflection => {
            val groupedRows =
              getRelationsGroupedRowsFrom(entityReflection, 9, 10)
                            ...
                            case result =>
                              result._1.length match {
                                case 0 =>
                                  Future.failed(
                                    OrganizationNotFoundException(
                                      s"Application ${application.id}:${application.label} might be orphan"
                                    )
                                  )
                                case 1 =>
                                ...
                                case _ =>
                                  Future.failed(
                                    DuplicateOrganizationException(
                                      s"Application ${application.id}:${application.label} has duplicate organization relation"
                                    )
                                  )
                              }
                          }
      ...
      case Failure(cause) =>
        Future.failed(ApplicationQueryExecutionException(cause))
    })
  }
  ...
  /** A method that gets all existing applications
    ...
    *   a future [[List List]] of [[Application Application]]
    */
  def getAllApplications(implicit
      ec: ExecutionContext
    ...
    getApplications(
      ...
      case Success(applications) =>
        applications.length match {
          case 0 =>
            Future.failed(NoEntryException("Application store is empty"))
  ...
  /** A method that fetches application by its id
    *
    * @param id
    *   the id of the application to be fetched
    * @param ec
    *   the '''implicit''' [[ExecutionContext ExecutionContext]] used to
    *   parallelize computing
    * @return
    *   a future [[Application Application]] which reflects application state
    ...
    getApplications(
      ...
      case Success(applications) =>
        applications.length match {
          case 0 =>
            Future.failed(
              ApplicationNotFoundException(s"Application $id couldn't be found")
            )
  ...
  /** A method that persist application state in the database
    *
    * @param application
    *   the application object to persist
    * @param ec
    *   the '''implicit''' [[ExecutionContext ExecutionContext]] used to
    *   parallelize computing
    * @return
              ...
              Future
                              ...
                              OrganizationStore
                                ...
                                .copy(id =
                                  application.organizations
                                    .filter(_._1 == true)
                                    .map(_._2._2.id.toString)
                                )
                            ),
                            orderBy = List(
                              (OrganizationStore.Column.ID(), 1)
                            )
                          )
                      )
                      .transformWith({
                        case Success(organizations) =>
                          ...
                          val orgs = organizations zip application.organizations
                            .sortBy(_._2._2.id)
                            .map(_._2._1)
                ...
                .transformWith({
                  case Success(organizations) =>
                    ...
                    organizationStore
                      .bulkPersistOrganizations(organizations.flatten)
                  case Failure(cause) => Future.failed(cause)
                })
            )
          )
  ...
  /** A method that persist several applications state in the database
    *
    * @param applications
    *   applications objects to persist
    * @param ec
    *   the '''implicit''' [[ExecutionContext ExecutionContext]] used to
    *   parallelize computing
    * @return
            ...
            List(
              Utils.igniteToScalaFuture(
                igniteCache.putAllAsync(
                  (applications
                    .map(_.id) zip applications).toMap[UUID, Application].asJava
                )
              ),
              Future
                .sequence(
                    ...
                    List(
                      organizationStore
                        ...
                        .transformWith({
                          case Success(organizations) =>
                            val orgs =
                              organizations zip application.organizations
                                .sortBy(_._2._2.id)
                                .map(_._2._1)
                            Future.successful(orgs.flatMap({ o =>
                              try {
      ...
      case Success(tx) =>
        ...
        Future
          .sequence(
            List(
              Utils.igniteToScalaFuture(
                igniteCache.removeAsync(application.id)
              ),
              organizationStore
                .getOrganizations(
                  OrganizationStore
                    ...
                    .copy(filters =
                      List(
                        OrganizationStore
                          .GetOrganizationsFilter()
                          .copy(id =
                            application.organizations.map(_._2._2.id.toString)
                          )
                      )
                    )
                )
...
object ApplicationStore {
  ...
  object Column {
    case class ID(val order: Int = 0, val name: String = "a.ID") extends GetEntityFilters.Column
  }
}
