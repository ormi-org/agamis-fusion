...
import io.agamis.fusion.core.db.models.sql.exceptions.EntityRelationConstraintViolationException
...
class Application(implicit @transient protected val store: ApplicationStore) extends Model {
    ...
    @transient
    private var _organizations: List[(Boolean, (Status, Organization, (FileSystem, String)))] = List()
