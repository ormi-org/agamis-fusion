...
import io.agamis.fusion.external.http.entities.File.DIRECTORY
import io.agamis.fusion.external.http.entities.File.FILE
import io.agamis.fusion.external.http.entities.nested.file.{Acl, Metadata}
import io.agamis.fusion.external.http.entities.nested.file.MetadataJsonProtocol._
import io.agamis.fusion.external.http.entities.nested.file.AclJsonProtocol._
...
trait FileJsonProtocol extends SprayJsonSupport with DefaultJsonProtocol {
  ...
  import io.agamis.fusion.external.http.actors.FileRepository._
  ...
  implicit object FileJsonFormat extends RootJsonFormat[File] {
    def write(f: File): JsObject = {
      ...
      f.chunkList match {
        case Some(chunkList) => values :+= ("chunkList" -> JsArray(chunkList.map(chunkId => JsString(chunkId.toString)).toVector))
        case None =>
      }
    ...
    def read(json: JsValue): File = {
      ...
      jsFileObject.getFields("name", "type", "metadata", "acl", "owner") match {
        ...
        case other => throw DeserializationException("Cannot deserialize File: File expected but got invalid input. Raw input: " + other)
      }
    }
  }
}
