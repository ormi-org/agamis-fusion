          ...
          pathPrefix("file")(
            concat(
              ...
              pathPrefix("path" / Segment) { path: String =>
                get {
                  onComplete(
                    buildFileRepository.ask(
                      FileRepository.GetFileByPath(path, _)
                    )
                  ) {
              ...
              pathPrefix("id" / Segment) { fileId: String =>
                concat(
                  ...
                  get {
                    onComplete(
                      buildFileRepository.ask(
                        FileRepository.GetFileById(fileId, _)
                      )
                    ) {
                      case Success(file) => complete(StatusCodes.OK -> file)
                      case Failure(reason) =>
                  ...
                  put {
                    entity(as[FileDto]) { file =>
                      ...
                      onComplete(
                        buildFileRepository.ask(
                          FileRepository.UpdateFile(file, _)
                        )
                      ) {
                  ...
                  delete {
                    ...
                    onComplete(
                      buildFileRepository.ask(
                        FileRepository.DeleteFile(fileId, _)
                      )
                    ) {
                      case Success(response) =>
                        response match {
